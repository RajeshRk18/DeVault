...
pub fn is_password_strong(mut pass: String) -> bool {
    ...
    pass.chars().for_each(|c| {
        ...
        has_number |= c.is_numeric();
        has_symbol |= symbols.contains(c);
    });
    let pass_len = pass.len();
    pass.zeroize();

    has_lowercase && has_uppercase && has_number && has_symbol && pass_len >= 12
}

pub fn bytes_to_num(bytes: &[u8]) -> u64 {
    let mut result = 0;
    let mut multiplier = 1;

    for &byte in bytes.iter().rev() {
        result += u64::from(byte) * multiplier;
        multiplier *= 2;
    }
    result
}

// These helpers must be added in frontend.
...
pub fn generate_seed_phrase(length: usize) -> String {
    let mut rng = FastRng::with_seed(u64::MAX);
    let seed_phrase: Vec<&str> = (0..length)
        .map(|_| {
            let rand_access = rng.u64(..WORDLIST.len() as u64);
            WORDLIST[rand_access as usize]
        })
        .collect();
...
pub fn generate_password(length: usize) -> String {
    let mut rand = FastRng::with_seed(u64::MAX);

    let password: String = (0..length)
        .map(|_| {
            let rand_val = rand.u64(..);
            CHARSET[rand_val as usize % CHARSET.len()] as char
        })
        .collect();
...
pub fn generate_password_checked(length: usize) -> String {
    let mut pass = generate_password(length);
    let mut is_strong = is_password_strong(pass.clone());

    loop {
        if is_strong {
            break;
        } else {
            pass = generate_password(length);
            is_strong = is_password_strong(pass.clone());
        }
    }
    pass
}*/
